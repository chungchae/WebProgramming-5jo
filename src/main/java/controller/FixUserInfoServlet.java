...
public class FixUserInfoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            // 회원정보 업데이트 쿼리
            String updateSql = "UPDATE User SET email = ?, password = ?, univ = ?, major = ?, grade = ?, hobby = ?, introduction = ? WHERE id = ?";
            try (PreparedStatement pstmt = conn.prepareStatement(updateSql)) {
                ...
                if (rowsUpdated > 0) {
                    out.println("<script>alert('회원정보가 성공적으로 수정되었습니다.'); location.href='/user/mypage';</script>");
                } else {
